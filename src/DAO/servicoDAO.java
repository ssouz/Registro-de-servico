...
public class servicoDAO {
    ...
    public void cadastrarServico(servicoDTO objfuncionariodto) {
        ...
        try {
            PreparedStatement pstm = new ConexaoDAO().conectaBD().prepareStatement(
                    "insert into servicos (nome_funcionario, nome_cliente, valor_servico, servico_descricao, data_servico) values (?,?,?,?,?)");
    ...
    public ArrayList<servicoDTO> consultarHistoricoServicos() {
        ...
        try {
            PreparedStatement pstm = new ConexaoDAO().conectaBD().prepareStatement("select * from servicos");
